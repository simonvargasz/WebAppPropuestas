                ...
                <div class="card bg-dark border-2 m-2 rounded-4" data-bs-theme="dark">
                  <div class="card-header border-2 rounded-4 shadow-sm m-0">
                    <div class="row p-0 m-0">
                      ...
                      <div class="col-4 p-0 m-0">
                        <h3 class="text-white text-center">Modificacion de estudiante: {{$estudiante->nombre}}</h3>
                      </div>
                    </div>
                  </div>
                  <form class="m-3" action="{{ route('admin.updateEstudiante',['rut' => $estudiante->rut]) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="form-floating">
                        <input class="form-control" id="name" placeholder="John" value="{{$estudiante->nombre}}" name="nombre">
                        <label for="name">Nombre</label>
                    </div>
                    <div class="form-floating mt-3">
                        <input class="form-control" id="lastname" placeholder="Doe" value="{{$estudiante->apellido}}" name="apellido">
                        <label for="lastname">Apellido</label>
                    </div>
                    <div class="form-floating mt-3">
                        <input class="form-control" id="lastname" placeholder="johndoe@gmail.com" value="{{$estudiante->email}}" name="email">
